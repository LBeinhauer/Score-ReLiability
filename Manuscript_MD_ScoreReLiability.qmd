...
heterogeneity, the REML-estimatore is used (ref). To identify which
...
estimates of Cohen’s d are corrected, according to @eq-dc, including the
corrected standard errors (@eq-SEdc). Thereby, estimates of ES are
...
In @fig-hs, the results of employing @eq-HS to compute the heterogeneity
(variance) in corrected ES is highlighted. All else held equal, only the
individual level of mean reliability \$ \mu\*{\*\\rho{xx‘}}\$ is varied
from .5 to .8. Mean corrected ES $\mu_{\delta}$ is held constant at 1,
its variance $\tau^2_{\delta}$ held constant at .04 and the variance of
score reliability $\tau^2_{\rho_{xx‘}}$ held constant at .02.
...
bracket is bound to be positive ($\frac{1}{R_1} – 1$). Similalry, $R_2$,
...
variance of 49.8% ($1 – R_2$) or more is necessary for scenario (b) to
no longer be true. The smaller the average score reliability, the
...
example, it is unlikely that an $R_2 = .51$ leads to a term that is not
