...
Heterogeneity in standardised effect sizes, such as Cohen’s d, is
informative because it is often interpreted as evidence that a
phenomenon varies across contexts or populations. In research synthesis,
one influential explanation for such variation is that it arises from
differences in measurement precision across studies, implying that
correcting effect sizes for attenuation due to score unreliability
should reduce between-study variability. We reanalyse data on a wide
range of phenomena from large-scale collaborative projects, including
the ManyLabs studies, Registered Replication Reports, and the
Psychological Science Accelerator. We apply conventional attenuation
correction procedures and propose an alternative framework that treats
standardised effect sizes as ratio variables to examine how differences
in score reliability relate to heterogeneity. Across datasets,
differences in score reliability are small and can not explain effect
size heterogeneity, contrary to prevailing expectations. For most
phenomena, attenuation correction procedures increased heterogeneity,
rather than decreasing it. We attribute this to between-study
differences in underlying score distributions that dominate the
comparatively small variation in random measurement error. These
findings imply that unreliability is unlikely to be a major driver of
effect size heterogeneity for phenomena comparable to the ones
reanalysed here. More broadly, the results suggest that meta-analysts
should not expect that effect size heterogeneity can be reduced or
explained by controlling for differences in random measurement error.
Our results call for greater attention to substantive sources of
heterogeneity, such as sampling practices or population differences,
when synthesising empirical research and interpreting heterogeneity in
meta-analytic models across studies and disciplines.

#### Highlights

##### What is already known?

-   Heterogeneity in standardised effect sizes is prevalent across most
    phenomena in the social sciences.

-   Score reliability tends to differ, if the same instrument is
    administered across different contexts, populations or timepoints.

-   Since score reliability affects standardised effect sizes, it seems
    sensible, that differences in score reliability introduce
    heterogeneity that can be corrected for.

##### What is new?

-   We demonstrate that, contrary to expectations, differences in score
    reliability do not serve as moderators for unexplained
    heterogeneity.

-   Instead, we show that correcting for score reliability routinely
    increases effect size heterogeneity.

-   Understanding standardised effect sizes as ratio variables, we show
    under which circumstances a meta-analyst can expect that correcting
    for score reliability can only increase effect size heterogeneity.

##### Potential impact for RSM readers

-   We highlight the importance of considering differences in underlying
    construct distribution as explanations for heterogeneity in both
    score reliability and standardised effect sizes, instead of
    differences in random measurement error.

-   The discussion clears popular misconceptions for how score
    reliability affects effect size heterogeneity and what meta-analysts
    can expect, if they use an attenuation correction to correct for
    differences in random measurement error.
...
expected replication rate [@vu2024].
...
such as the Many Labs studies or Registered Replication Reports (e.g.
[@ManyLabs1; @RRRHart]) for the first time allow researchers to estimate
heterogeneity undistorted by selection processes like publication bias.
In re-analyses of these studies, Olsson-Collentine et al. [-@olsson2020]
identify a positive correlation between a phenomenon’s effect size and
its heterogeneity. Both van Erp et al. [-@vanErp2017] and Stanley et al.
[-@stanley2018] estimate strong degrees of heterogeneity across
conceptual replications in Psychology. In a separate re-analysis of
large scale direct replications, Renkewitz et al. (in preparation)
identify substantial heterogeneity in almost all projects where a
non-zero effect could be identified. This aligns with the correlation
found by Olsson-Collentine et al. [-@olsson2020].

The search for moderators that might explain said heterogeneity across
several phenomena has not been particularly successful [@ManyLabs3,
...
standardised effect sizes, claim that differences in score reliability
...
standardised effect sizes (from here-on abbreviated as ES) such as
...
publication-bias and sensitivity analyses*” [-@wiernik2020, p. 3].
...
potential moderator effects*” [-@wiernik2020, p. 4]. They base their
...
-   To what extent can heterogeneity in standardised effect sizes be
...
[osf.io/7p8rk/](https://osf.io/7p8rk/){.uri}, the DRIPHT repository. A
documentation of the standardised format, including a web-application to
easily explore the public data-sets, can be found in Fünderich et al.
...
standardised format when the DRIPHT repository was set up [@dripht2024].
...
For the standardised mean difference $d$, we can compute the standard
...
these standardised effect sizes attempt to describe their population
...
without criticism [@sijtsma2009], Cronbach’s Alpha is used to estimate
...
generate estimates of standardised ES, both uncorrected and corrected,
...
be made responsible for heterogeneity in standardised effect sizes.
...
estimate of mean score reliability; $\hat{\tau}_{\rho_{XX'}}$ is the
...
are related to differences in standardised effect sizes. Therefore,
...
in standardised effect sizes, as described in H&S and W&D.
...
higher-order moments are negligible [@vandervaart2000]. Here we use the
...
Substituting for ES, unstandardised mean difference MD and pooled
...
differences in standardised differences ES as well, but it can not be
...
heterogeneity in score variance; $\hat{\tau}_{{\sigma^2_T}}$ represents
the estimate of heterogeneity in true score variance.

## Discussion

We have examined meta-analytic heterogeneity in standardised mean
...
heterogeneity, as we observe for standardised mean differences. Only as
...
standardised effect sizes are not identical, the way score
...
standardised effect sizes and why our phenomena vary across settings,
...
The scripts to automatically access the data and reproduce the analyses
underlying the findings of this paper are openly available on the OSF at
[osf.io/yuas9](https://osf.io/yuas9/), 10.17605/OSF.IO/YUAS9.
